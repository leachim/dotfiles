...
class Py3status:
    ...
    cache_timeout = 1
    format = "[\?color=cpu {cpu_temp}°C - {cpu_usage}%]"
    #format = "[\?color=cpu {cpu_temp}°C - {cpu_usage}%], " \
    #         "[\?color=mem Mem: {mem_used}/{mem_total} GB ({mem_used_percent}%)]"
    mem_unit = 'GiB'
    swap_unit = 'GiB'
    temp_unit = u'°C'
    thresholds = [(0, "good"), (70, "degraded"), (80, "bad")]
    zone = None
    ...
    def sysData(self):
		...
		## CUSTOM MODIFICATION, only show information when temperature above threshold
        if self.data.cpuTemp(self.zone, self.temp_unit) < 60:
            response = {
